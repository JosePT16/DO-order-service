...
pipeline {
    ...
    environment {
        SERVICE_NAME = 'do-order-service'
        IMAGE_REPO = 'joseptz/do-order-service'
        DOCKERFILE_PATH = 'Dockerfile'
        K8S_DEPLOYMENT_NAME = 'order-service'
        K8S_GREEN_DEPLOYMENT_NAME = 'order-service-green'
        K8S_CONTAINER_NAME = 'order-service'
        K8S_APP_LABEL = 'order-service'
        K8S_STABLE_SERVICE_NAME = 'order-service'
        K8S_NODEPORT_SERVICE_NAME = 'order-service-nodeport'
    }
    ...
    stages {
        ...
        stage('Test') {
            steps {
                powershell 'if (Test-Path ".\\node_modules\\.bin\\jest.cmd") { npm test -- --runInBand --passWithNoTests } else { Write-Host "Skipping tests because jest is not installed in this repo yet." }'
            }
        }
